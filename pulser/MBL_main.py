...
from pulser.routines_MBL import *
import pulser.init_MBL as iMBL
import csv
import json
...
def main_MBL(indices_array):
    ...
    for r in register_dict.values():
        ...
        r['reg'] = reg_from_W_list(r['W_list'])
...
def dict_write(dict,csv_columns = ['No','W_list','para','score'],index = 0):
    csv_file = "Results{}.csv".format(index)
    try:
        with open(csv_file, 'w') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
            writer.writeheader()
            for i in dict.keys():
                writer.writerow({'No':i,'W_list':dict[i]['W_list'],'para':dict[i]['para'],'score':dict[i]['score']})
    except IOError:
        print("I/O error")